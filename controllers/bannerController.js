...
const editModifyBanner = async (req, res) => {
  try {
    const existingBanner = await banner.findById(req.body.id);

    if (req.files.length !== 0) {
      existingBanner.name = req.body.bName;
      existingBanner.image = req.files.map((x) => x.filename);
      existingBanner.description = req.body.bDescription;
    } else {
      existingBanner.name = req.body.bName;
      existingBanner.description = req.body.bDescription;
    }
